...
public class InMemoryLuceneHandler implements Queryable, Serializable {
	...
	public InMemoryLuceneHandler(TextCollection collection){
	...
	public List<ScoredDocument> search(Query query, long maxNumberOfDocuments) {
		...
		try {
			...
			for (ScoreDoc d : result.scoreDocs) {
				ret.add(new ScoredDocument(new Document(this.collection, Integer.valueOf(getSearcher().doc(d.doc).get("myId"))), d.score));
			}
			return ret;
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void addDocument(Document document) {
		...
		if (!document.getCollection().equals(collection)){
			try {
				throw new UnsupportedDatasetException("This sample should only include documents from a unique collection");
